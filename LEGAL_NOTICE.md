# LEGAL_NOTICE.md

## Product identity
**CMO-VIH Pro v3 — September 2026**

## Intended purpose
This software is intended as a semi-clinical decision-support tool for HIV pharmaceutical care, stratification support, pilot workflows, and research-oriented operational use.

## Clinical disclaimer
The software provides decision support only and must not be used as a substitute for clinician judgment, multidisciplinary review, or direct patient assessment.

## AI disclaimer
AI-assisted extraction and heuristic interpretation may contain omissions, classification errors, or incorrect inferences. All AI-derived content requires human validation before use.

## Regulatory positioning
This software is **not** a certified medical device and does not claim regulatory clearance, approval, or conformity assessment.

## Liability boundary
The tool is provided for support, research, and pilot deployment contexts. Users are responsible for validating all outputs and ensuring compliance with local policies, procedures, and professional obligations.

## Intellectual property
CMO-VIH Pro v3 — September 2026. All rights reserved. No reproduction, redistribution, or derivative commercial use without permission.
